...
#### fix. TUI 컨텍스트 컴팩션 중복 알림 및 잘못된 알림 제거
- 날짜: 2025-11-11 16:20 (Asia/Seoul)
- 컨텍스트: 응답 완료 시마다 "컨텍스트 컴팩션 발생: 캐시 재생성 (감소: 0 tokens)" 메시지가 2번씩 중복 표시되는 문제. 감소 토큰이 0인 경우 의미 없는 알림이며, 중복 알림도 사용자 경험을 저해함.
- 변경사항:
  - `src/presentation/tui/services/session_manager.py` (라인 344-399): `_detect_compaction()` 메서드 개선
    * `cache_creation` 트리거: 토큰 감소가 0보다 클 때만 이벤트 생성 (라인 378: `if cache_creation_tokens > 0 and reduction_tokens > 0`)
    * `reduction_tokens` 계산을 조건 체크 전으로 이동하여 모든 트리거에서 사용 가능하도록 개선
  - `src/presentation/tui/app.py` (라인 141-143, 976-1017): 중복 알림 방지 로직 추가
    * 인스턴스 변수 추가: `_last_compaction_event`, `_compaction_debounce_seconds` (5초)
    * `_create_usage_callback()` 내 컴팩션 알림 처리 개선:
      - 감소 토큰이 0 이하인 경우 즉시 리턴 (라인 981-982)
      - 최근 5초 내 동일 트리거의 컴팩션 이벤트는 중복으로 간주하여 무시 (라인 984-1002)
      - 마지막 컴팩션 이벤트 추적하여 디바운싱 적용 (라인 1017)
      - 숫자 포맷팅 개선: `{reduction:,}` 천 단위 콤마 표시 (라인 1012)
...
#### feat. TUI UX 개선 - 입력 이력, 완료 알림, 통계 표시, 토큰 표시, 컨텍스트 경고
- 날짜: 2025-11-12 (Asia/Seoul)
- 컨텍스트: TUI 기능명세서를 기반으로 사용자 경험을 개선하기 위한 5가지 핵심 기능 추가. 기존 기능들이 더 잘 작동하도록 개선.
- 변경사항:
  1. **입력 이력 탐색 개선** (`src/presentation/tui/components/input_box.py`):
     - 입력 중에도 ↑/↓ 키로 이력 탐색 가능
     - 원래 입력을 임시 저장 (`temp_input`) 후 복원
     - 이력 탐색 전: 현재 입력 저장 → 이력 탐색 → ↓로 원래 입력 복원
  2. **응답 완료 알림** (`src/presentation/tui/components/chat_view.py`, `src/presentation/tui/app.py`):
     - `ChatView.add_completion_message()` 메서드 추가
     - Worker 정상 완료 시 "✓ 응답 완료 (토큰 수)" 메시지 표시
     - 비주얼 피드백으로 완료 상태 명확히 전달
  3. **세션 종료 시 통계 표시** (`src/presentation/tui/app.py`):
     - `action_quit()` 개선: 종료 시 세션 통계를 사용자에게 표시
     - 메시지 수, 총 토큰 사용량 (입력/출력), 예상 비용 표시
     - 구조화된 통계 박스로 가독성 향상
  4. **메시지별 토큰 표시** (`src/presentation/tui/components/chat_view.py`, `src/presentation/tui/app.py`):
     - `add_user_message()`, `add_assistant_message()`에 `tokens` 파라미터 추가
     - 각 메시지 옆에 토큰 수 표시 (타임스탬프와 함께)
     - 세션 불러오기 시에도 과거 메시지의 토큰 정보 표시
  5. **컨텍스트 윈도우 80% 경고** (`src/presentation/tui/app.py`):
     - 컨텍스트 윈도우 사용량 80% 도달 시 한 번만 경고 메시지 표시
     - 현재 사용량, 권장 조치 (새 세션 시작 등) 안내
     - 새 세션 시작 시 경고 플래그 리셋
- 영향범위: 기능 개선, UX 향상
- 테스트:
  - 입력 이력: 일부 입력 후 ↑/↓로 이력 탐색 및 복원 확인
  - 응답 완료: 메시지 전송 후 "✓ 응답 완료" 메시지 표시 확인
  - 세션 통계: Ctrl+Q로 종료 시 통계 표시 확인
  - 메시지별 토큰: 각 메시지 옆에 토큰 수 표시 확인
  - 컨텍스트 경고: 사용량 80% 도달 시 경고 메시지 표시 확인
- 후속 조치:
  - 세션 검색 기능 추가 (날짜/키워드 필터)
  - 로그 파일 관리 UI 구현
  - 입력창 자동 크기 조절 (최소 3줄, 최대 10줄)

#### refactor. TUI 스마트 피드백 루프 LLM 기반 조건 생성으로 전환
- 날짜: 2025-11-11 (Asia/Seoul)
- 컨텍스트: 기존 하드코딩된 템플릿 기반 조건 생성을 LLM 기반으로 전환하여 사용자 요청에 맞는 맞춤형 평가 조건 생성. Haiku 4-5 모델을 사용해 빠르고 정확한 조건 생성.
- 변경사항:
  - `src/presentation/tui/services/smart_feedback_loop.py`:
    * **하드코딩 템플릿 제거**: `CONDITION_TEMPLATES` 딕셔너리 삭제
    * **AutoConditionGenerator 클래스 개선**:
      - `extract_keywords()`, `get_template_condition()` 메서드 제거
      - `generate_condition()` 메서드를 항상 LLM 호출하도록 수정
      - 기존 템플릿들을 프롬프트의 "예시"로 통합하여 품질 향상
    * **LLM 프롬프트 개선**:
      - 작업 유형 분석 단계 추가 (테스트, 문서, 코드, 리팩터링, 버그 수정 등)
      - 5가지 예시 템플릿 제공하여 구체적인 기준 생성 유도
      - 사용자 요청 맥락에 맞는 맞춤형 조건 생성
    * **prepare_condition() 메서드 개선**:
      - `semi-auto` 모드도 LLM이 간단한 목표를 구체적인 조건으로 확장
      - 모든 모드에서 LLM 기반 조건 생성 활용
    * **모델**: `claude-haiku-4-5-20251001` 유지 (빠른 조건 생성)
- 영향범위: 기능 개선, 자동화 향상
- 테스트: TUI에서 스마트 피드백 루프 활성화 후 다양한 요청으로 조건 생성 품질 확인 필요
- 후속 조치:
  - 조건 생성 품질 모니터링
  - 사용자 피드백 기반 프롬프트 개선
